...
fig1 = Figure(201,'size',[60,45]); hist(distances,linspace(0,600,20)); h = findobj(gca,'Type','patch'); set(h,'FaceColor','w'); xlim([-50,600]); fig1.cleanup
fig2 = Figure(202,'size',[60,45]); hist(dist_x,linspace(0,600,20),'FaceColor','w');h = findobj(gca,'Type','patch'); set(h,'FaceColor','w'); xlim([-50,600]); fig2.cleanup

bins = linspace(150,500,6);
bins_x = [25,50,linspace(100,200,4)];
...
if tosave
    fig.save(['plots/ConnDist_' layer_from '_' layer_to '.eps']);
end
...
ylim([0,yLim]); xlim([0,max(bins_x)+50]);
xlabel('Intersomatic distance x(\mum)');
ylabel('Connection Probability');
fig.cleanup;

if tosave
    fig.save(['plots/ConnDist_x_' layer_from '_' layer_to '.eps']);
end
...
if tosave
    fig.save(['plots/ConnDist_y_' layer_from '_' layer_to '.eps']);
